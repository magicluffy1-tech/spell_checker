...
- 입력: 엑셀/CSV 업로드 OR 구글 시트 공유 링크(읽기 전용)
  → 구글 시트 모드: 탭(시트) 이름을 직접 입력하면 해당 탭만 로드
...
for key, default in {
    ...
    "done": False,
    "sheet_tabs": [],           # 조회된 탭 목록
    "sheet_url_cached": "",     # 탭 목록 조회에 사용된 URL (재조회 방지)
    "selected_sheet_name": "",  # 선택된 탭 이름
}.items():
...
with col_load_1:
    ...
    # ── 구글 시트 링크 모드 ──────────────────────
        ...
        # URL이 입력됐을 때 탭 목록 자동 조회
        if sheet_url:
            # URL이 바뀌었을 때만 재조회
            if sheet_url != st.session_state["sheet_url_cached"]:
                with st.spinner("시트 탭 목록 조회 중..."):
                    tabs = get_sheet_tabs(sheet_url)
                    st.session_state["sheet_tabs"] = tabs
                    st.session_state["sheet_url_cached"] = sheet_url
            ...
            if tabs:
                ...
                # 탭 목록 조회 실패 시 직접 입력
                st.info("ℹ️ 탭 목록을 자동으로 가져오지 못했습니다. 탭 이름을 직접 입력하세요.")
                sheet_name_input = st.text_input(
                    "탭(시트) 이름 입력",
                    value="",
                    placeholder="예: 창체_완료",
                    help="구글 시트 하단에 표시되는 탭 이름을 정확히 입력하세요.",
                )
            ...
            if load_btn:
                with st.spinner(
                    f"'{sheet_name_input}' 탭 로드 중..." if sheet_name_input else "첫 번째 탭 로드 중..."
                ):
                    try:
                        df_loaded, _ = get_raw_data_from_public_url(
                            sheet_url,
                            sheet_name=sheet_name_input if sheet_name_input else None,
                        )
                        st.session_state["selected_sheet_name"] = sheet_name_input
                        tab_label = f"'{sheet_name_input}' 탭" if sheet_name_input else "첫 번째 탭"
                        st.success(
                            f"✅ {tab_label} 로드 완료: {len(df_loaded)}행 × {len(df_loaded.columns)}열"
                        )
            ...
            # 버튼 누르지 않아도 이미 세션에 데이터가 있으면 유지
            if df_loaded is None and st.session_state["df_original"] is not None:
                df_loaded = st.session_state["df_original"]
...
with col_load_2:
    ...
    if input_mode == "구글 시트 공유 링크":
        st.info(
            ...
            "2. `창체_완료`, `행발_완료` 등 원하는 탭 이름을 선택하거나 직접 입력하세요.\n"
...
2. **탭 이름이 정확한가요?**
   - 탭 이름에 공백이나 특수문자가 포함된 경우 드롭다운으로 선택하세요.
   - 탭 이름은 구글 시트 하단 탭에 표시된 이름과 완전히 동일해야 합니다.
...
if run_btn and can_run:
    ...
    for i, record in enumerate(records):
        ...
        reasons_list = [
            f"[{err['original']} -> {err['corrected']}] {err['reason']}"
        ...
        if (i + 1) % chunk_delay_every == 0 and (i + 1) < total:
            status_area.warning(
                f"⏸️ 서버 보호 장치: {chunk_delay_every}줄 처리 완료 - 3초간 안전 휴식 중..."
            )
            time.sleep(3)
        else:
  ...
  <div class="metric-value" style="color: #eab308;">{total_errors}개</div>
    ...
    if st.button("🔄 새로운 문서 또는 시트로 처음부터 다시 시작", use_container_width=True):
        for key in ["df_original", "df_result", "text_column", "processing", "done",
                    "sheet_tabs", "sheet_url_cached", "selected_sheet_name"]:
            default_false = {"processing", "done"}
            default_list  = {"sheet_tabs"}
            default_str   = {"sheet_url_cached", "selected_sheet_name"}
            if key in default_false:
                st.session_state[key] = False
            elif key in default_list:
                st.session_state[key] = []
            elif key in default_str:
                st.session_state[key] = ""
            else:
